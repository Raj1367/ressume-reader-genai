...
import { ANTHROPIC_TOOL_BETAS, AnthropicToolExtrasSchema, handleToolChoice } from "./utils/tools.js";
import { _convertMessagesToAnthropicPayload, applyCacheControlToPayload } from "./utils/message_inputs.js";
...
var ChatAnthropicMessages = class extends BaseChatModel {
	...
	*/
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields ?? {});
		this._addVersion("@langchain/anthropic", "1.3.22");
	...
	*/
	invocationParams(options) {
		...
		const toolBetas = options?.tools?.reduce((acc, tool) => {
			if (typeof tool === "object" && "type" in tool && tool.type in ANTHROPIC_TOOL_BETAS) {
	...
	*/
	...
	async *_streamResponseChunks(messages, options, runManager) {
		...
		if (options.cache_control) formattedMessages = applyCacheControlToPayload(formattedMessages, options.cache_control);
		const payload = {
			...params,
			...formattedMessages,
			stream: true
	...
	async _generateNonStreaming(messages, params, requestOptions, cacheControl) {
		let formattedMessages = _convertMessagesToAnthropicPayload(messages);
		if (cacheControl) formattedMessages = applyCacheControlToPayload(formattedMessages, cacheControl);
	...
	async createStreamWithRetry(request, options) {
		...
		const makeCompletionRequest = async () => {
			try {
				...
				throw wrapAnthropicClientError(e);
			}
		};
	...
	*/
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "jsonSchema") {
			...
			} else if (typeof schema.name === "string" && typeof schema.description === "string" && typeof schema.input_schema === "object" && schema.input_schema != null) {
				tools = [schema];
			...
			outputParser = createFunctionCallingParser(schema, functionName, AnthropicToolsOutputParser);
			if (this.thinking?.type === "enabled" || this.thinking?.type === "adaptive") {
		...
		return assembleStructuredOutputPipeline(llm, outputParser, includeRaw, includeRaw ? "StructuredOutputRunnable" : "ChatAnthropicStructuredOutput");
	}
};
